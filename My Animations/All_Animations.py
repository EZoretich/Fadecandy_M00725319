...
#------------------------------------------------------------------ FUNCTION FOR VIDEOMAKER EFFECT
#-------------------------------------------------------Function for Frame "Fading" Effect
...
#------------------------------------------------------------------ FUNCTION FOR COLORFUL PIRAMIDS
#-------------------------------------------------------Function to create triangles changing colors (at random)
def Triangle(*argv): #Create a function with arguments
    led = int(argv[0]) # to which specific index is assign
    start = int(argv[1])
    stop = int(argv[2])
    step = int(argv[3])
    for rows in range(start,stop,step):
        for i in range (3-rows): #Piramids are going to be quite small (they occupy 3 rows)
            R = random.randint(i,i+360) #Each R G B val will be random
            G = random.randint(i,i+360) #i-0 and i+360, to that the colors will stay
            B = random.randint(i,i+360) #on the brighter side 
            led_wall[led+(i+rows)*60] = (R,G,B) #Assign vals in tuple to main list
...
while True:
    ...
    if choice == 1: # if input is 1: Play first anymation
        ...
        while True:
            if choose.isdigit() == True: #if the input is a number:
                choose = int(choose)
                if choose <= 0 or choose > 8: #if the number is not between 1 and 8:
                    choose = input("Please select a number from the list! \n What's your choice? ")
                    continue
                else:
                    break

            else:
                choose = input("Not a number! Please insert the number corresponding to you favourite color: ")

        if choose == 1: #GREEN (All rows scrolling)
            Wave(0,255,0)

        if choose == 2: # PURPLE (All rows scrolling)
            Wave(127,0,255)
                
        if choose == 3: # BLUE (All rows scrolling)
            Wave(0,0,255)

        if choose == 4: # GREY (All rows scrolling)
            Wave(150,150,150)

        if choose == 5: # RED (All rows scrolling)
            Wave(255,0,0)

        if choose == 6: # YELLOW (All rows scrolling)
            Wave(255,255,0)

        if choose == 7: # ORANGE (All rows scrolling)
            Wave(255,128,0)

        if choose == 8: # PINK (All rows scrolling)
            Wave(255,102,178)
    ...
    #----------------------------------------------------------------ANIMATION 5: VIDEOMAKER EFFECTS
    #----- In this animation I recreated a couple of transitions
    #----- which were very popular 
    #----- among people that were using Videomaker
    #----- The first animation will clear the screen,
    #----- following the simulation frame from outer side to inner side;
    #----- the second animation will present a green line moving from left to right.
    #----- Each time the line will touch one of the two extremities (left/right),
    #----- the screen will add a purple line in the opposite direction that the line touched,
    #----- creating a "push" effect which will fill the screen before ending.
    if choice == 5:
        print("VideoMaker Transitions: A smashing blast from the past!")
        led_wall = [(250,250,250)]*360 # Make screen all white
        client.put_pixels(led_wall)

        Frame_Anim(0,60,1) # Row = 0 (1); from left to right
        Frame_Anim(119,360,60) # From row 0 to row 5 (1-6) ; top to bottom
        Frame_Anim(359,300,-1) #Row = 5 (6); from right to left
        Frame_Anim(300,119,-60) # From row 5 to row 1 (6-2) ; bottom to top
        Frame_Anim(60,118,1) # Row = 1 (2); from left to right
        Frame_Anim(118,358,60) # From row 1 to row 4 (2-5); top to bottom
        Frame_Anim(298,241,-1) # Row = 4 (5); from rigth to left
        Frame_Anim(241,122,-60) # From row 4 to row 2 (5-3); bottom to top
        Frame_Anim(121,179,1) # Row = 2 (3); from left to right
        Frame_Anim(238,181,-1) # Row = 3 (4); from right to left
        ...
        while True: #keep running
            ...
            time.sleep(0.0001)
    #----------------------------------------------------------------ANIMATION 6: COLORFUL PIRAMIDS
    #------------ This animatin will display 6 small piramids between the first three rows,
    #------------ appearing from left to right, and 6 more between the last three rows,
    #------------ appearing from right to left. All piramids will have random colors
    #------------ that will change at each iteration.
    if choice == 6:
        delay = 10 #set delay
        end_time = time.time() + delay # set end time
        while True: # keep running
            for x in range(0,60,20): # start at led 0, limit at led 60, step by 20 so piramids are far enough from each other
                Triangle(x+0,5,-1,-1)
                Triangle(x+5,0,6,1)
                Triangle(x+10,5,-1,-1)
                Triangle(x+15,0,6,1)
            if time.time() > end_time: # in time passed is more than end time
                break # break the loop
